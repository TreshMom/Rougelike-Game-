...
public:
    void update(EventManager&, EntityManager& em, SystemManager&, sf::Time t) {
		...
		if (lf || r || up || down ) { 
             em.update<PlayerComponent,MoveComponent>([&,r,lf,up,down]
            (auto& entity, PlayerComponent& pl, MoveComponent& mv){
                mv.data.x = [=,v = (rand() % 1000)/100.0, rs = t.asMilliseconds()](double tm){
                return 6*(r - lf) * std::exp((rs - tm)/600.0);
            };
                mv.data.y = [=,v = (rand() % 1000)/100.0, rs = t.asMilliseconds()](double tm){

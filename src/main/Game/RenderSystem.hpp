    ...
    void update(ECS::EventManager&, ECS::EntityManager& em, ECS::SystemManager&, sf::Time) {
        ...
        em.update_by_p<SpriteComponent>(
            ...
            [&](auto const& left, auto const& right) {
                int left_p = (~static_cast<int>(em.template has_component<GridComponent>(left)) << 1) |
                             static_cast<int>(em.template has_component<MoveComponent>(left));
                int right_p = (~static_cast<int>(em.template has_component<GridComponent>(right)) << 1) |
                              static_cast<int>(em.template has_component<MoveComponent>(right));

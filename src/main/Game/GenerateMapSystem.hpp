...
public:
    void update(EventManager& evm, EntityManager& em, SystemManager&,
        ...
        if (!created) {
            ...
            ptr_map->get_component<GridComponent>().data.right_down = {
                WORLD_WIDTH, WORLD_HEIGHT};
            ptr_map->get_component<GridComponent>().data.grid_density =
                GRID_DENSITY;
            ptr_map->get_component<GridComponent>().data.mesh.resize(
                GRID_DENSITY + 1, std::vector<ECS::EntityId>(GRID_DENSITY + 1));
            ...
            created = true;
            createWall(em, ptr_map, {0, 0},
                       sf::IntRect(0, 0, WINDOW_WIDTH, 2 * SPRITE_SIZE),
                       "src/main/Assets/tile_0040.png");
    ...
    void createWall(EntityManager& em, auto& map_ptr,
                    std::pair<int, int> position, sf::IntRect rect,
                    std::string texture_path) {
        auto ptr_wall = em.allocEntity<WallEntity>();
        em.update_by_id<SpriteComponent, PositionComponent>(
            ptr_wall->get_id(), [&](auto& entity, SpriteComponent& shapeData,
                ...
                shapeData.data.sprite.setPosition(position.first,
                                                  position.second);
                shapeData.data.sprite.setScale(2, 2);
                pos.data.x = position.first;
                pos.data.y = position.second;

                auto to_x_map = ECS::to_x(
                    map_ptr->template get_component<GridComponent>().data);
                auto to_y_map = ECS::to_y(
                    map_ptr->template get_component<GridComponent>().data);

                std::pair<int, int> x_bound = {
                    to_x_map(pos.data.x),
                    to_x_map(pos.data.x +
                             shapeData.data.sprite.getGlobalBounds().width)};
                std::pair<int, int> y_bound = {
                    to_y_map(pos.data.y),
                    to_y_map(pos.data.y +
                             shapeData.data.sprite.getGlobalBounds().height)};
                for (int x_ind = x_bound.first; x_ind <= x_bound.second;
                     x_ind++) {
                    for (int y_ind = y_bound.first; y_ind <= y_bound.second;
                         y_ind++) {
                        map_ptr->template get_component<GridComponent>()
                            .data.mesh.at(x_ind)
                            .at(y_ind) = entity.get_id();
                    }
                }
            });
    }

        ...
        while (!q.empty()) {
            ...
            em.update_by_id<PositionComponent, SpriteComponent, AttackComponent>(
                attack_mob_id, [&](auto& eid, PositionComponent& ps, SpriteComponent& sc, AttackComponent& ac) {
                    ...
                    em.update_by_id<PlayerComponent, SpriteComponent, PositionComponent, HealthComponent,
                                    MoveComponent>(
                        defend_id,
                        [&](auto& ent, PlayerComponent&, SpriteComponent& player_sprite,
                            ...
                            // player_mc.data.x = [&player_mc, vector_between, rs = t.asMilliseconds() / 1000.0](double
                            // tm) {
                            //     tm /= 1000;
                            ...
                            // };
                            // player_mc.data.y = [&player_mc, vector_between, rs = t.asMilliseconds() / 1000.0](double
                            // tm) {
                            //     tm /= 1000;
                            //     double alpha = sigmoid(tm, 3, rs);

...
public:
    void update(EventManager &, EntityManager &em, SystemManager &, sf::Time) override {
        ...
        em.update<InventoryComponent, AttackComponent, HealthComponent, PlayerComponent, ExperienceComponent>(
                [&](auto &, InventoryComponent &inv, AttackComponent &attack, HealthComponent &health,
                    PlayerComponent &, ExperienceComponent &exp) {
                    ...
                    em.update<SpriteComponent, MenuComponent>([&](auto &, SpriteComponent &sprite, MenuComponent &) {
                        sprite.data.text.setString(
                                get_pretty_string(attack.data, health.data, exp.data).str());
                    });
                });
    }
    ...
    std::stringstream get_pretty_string(const AttackData &attack, const HealthData& hp, const ExperienceData &exp) {
        std::stringstream ss;
        ss << std::fixed << std::setprecision(2) << std::setiosflags(std::ios::left)
           << std::setw(11) << "Level:" << std::setw(20) << exp.level_
           << std::setw(10) << "Exp:" << std::setw(20) << exp.current_exp_ << "\n\n"

           << std::setw(11) << "Dmg:" << std::setw(20) << attack.damage
           << std::setw(10) << "Rad:" << std::setw(13) << attack.attack_radius << "\n\n"

           << std::setw(10) << "Health:" << std::setw(15) << hp.hp
           << std::setw(10) << "Reg:" << std::setw(20) << hp.reg;
        return ss;
    }

...
struct GridInventoryData {
    ...
    GridInventoryData(double w, double h, uint32_t N_w, uint32_t N_h, Vec2 coords)
        : width(w), height(h), N_width(N_w), N_height(N_h), local_left_up_coords(std::move(coords)) {}

    GridInventoryData(GridInventoryData&& other) noexcept
        : is_exists(other.is_exists), width(other.width), height(other.height), N_width(other.N_width),
          N_height(other.N_height), local_left_up_coords(std::move(other.local_left_up_coords)) {}
...
struct MenuData {
    ...
    MenuData(MenuData&& other) noexcept
        : wear_grid(std::move(other.wear_grid)), backpack_grid(std::move(other.backpack_grid)) {}
